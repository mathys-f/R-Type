...
namespace net
{
	...
	enum class CommandId : std::uint8_t
	{
		k_req_login = 0x01,
		k_res_login = 0x02,
		k_req_join_room = 0x03,
		k_res_room_state = 0x04,
		k_client_input = 0x10,
		k_server_entity_state = 0x11,
		k_server_player_death = 0x20,
		k_server_score_update = 0x21,
		k_ack = 0xFF
	};
	...
	{
		return static_cast<std::byte>(static_cast<unsigned char>(v & 0xFFU));
	}
	...
	struct PacketHeader
	{
		...
		std::uint16_t m_fragmentId = 0;
		std::uint8_t m_fragmentIndex = 0;
		std::uint8_t m_fragmentCount = 0;
	...
	struct ReliabilityConfig
	{
		...
		std::chrono::milliseconds initialRto{200};
		std::chrono::milliseconds maxRto{2000};
	...
	public:
		 ...
		 */
		[[nodiscard]] std::optional<std::chrono::milliseconds> time_until_next_timeout(std::chrono::steady_clock::time_point now) const;
		/**
