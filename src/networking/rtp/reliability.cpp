...
{
    m_queue.push_back(Pending{.m_packet=packet, .m_last_sent=now, .m_attempts=1, .m_rto=m_config.initialRto});
}
...
std::vector<Packet> ReliableSendQueue::collect_timeouts(std::chrono::steady_clock::time_point now)
{
    ...
    for (auto it = m_queue.begin(); it != m_queue.end();)
    {
        ...
        if (pending.m_attempts >= m_config.maxRetransmissions)
        {
        ...
        const auto k_elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(now - pending.m_last_sent);
        if (k_elapsed >= pending.m_rto)
        {
            pending.m_last_sent = now;
            ++pending.m_attempts;
            pending.m_rto = std::min(pending.m_rto * 2, m_config.maxRto);
