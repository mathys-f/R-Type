...
namespace net {
Session::Session(asio::io_context& context, const asio::ip::udp::endpoint& remote, ReliabilityConfig config,
                 std::uint16_t localPort)
    : m_transport(std::make_shared<UdpTransport>(context, localPort)), m_config(config), m_send_queue(config),
      m_retransmit_timer(context) {
...
std::uint32_t Session::fragment_and_send(Packet packet, const asio::ip::udp::endpoint& endpoint, bool reliable) {
    ...
    const std::size_t k_fragment_count = (k_total_size + k_fragment_size - 1) / k_fragment_size;
    ...
    for (std::size_t i = 0; i < k_fragment_count; ++i) {
        ...
        fragment.header.m_flags = set_flag(fragment.header.m_flags, PacketFlag::KFragment);
        fragment.header.m_fragment_id = k_fragment_id;
        fragment.header.m_fragment_index = static_cast<std::uint8_t>(i);
        fragment.header.m_fragment_count = static_cast<std::uint8_t>(k_fragment_count);
...
std::optional<Packet> Session::ingest_fragment(Packet packet) {
    ...
    if (packet.header.m_fragment_count == 0 || packet.header.m_fragment_index >= packet.header.m_fragment_count ||
        static_cast<std::size_t>(packet.header.m_fragment_count) > k_max_fragments) {
        return std::nullopt;
    }
    ...
    if (k_new_buffer || buffer.m_parts.size() != packet.header.m_fragment_count) {
        ...
        buffer.m_reliable = has_flag(packet.header.m_flags, PacketFlag::KReliable);
        buffer.m_created_at = std::chrono::steady_clock::now();
        buffer.m_total_bytes = 0;
    }

    const std::size_t k_index = packet.header.m_fragment_index;
...
Packet Session::rebuild_packet(FragmentBuffer& buffer) {
    ...
    assembled.header.m_flags = clear_flag(assembled.header.m_flags, PacketFlag::KFragment);
    assembled.header.m_fragment_id = 0;
    assembled.header.m_fragment_index = 0;
    assembled.header.m_fragment_count = 0;
