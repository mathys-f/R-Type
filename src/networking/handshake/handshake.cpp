...
{
    Packet packet{};
    packet.header.m_command = static_cast<std::uint8_t>(CommandId::k_req_login);
    ...
    const std::uint16_t k_pref = (req.m_preferred_fragment_size == 0 ||
        req.m_preferred_fragment_size > k_max_payload_size)
    ...
    std::ranges::transform(trimmed, std::back_inserter(packet.payload), [](char c) {
...
{
    Packet packet{};
    packet.header.m_command = static_cast<std::uint8_t>(CommandId::k_res_login);
    packet.header.m_flags = 0;

    const std::uint16_t k_effective = (res.m_effective_fragment_size == 0 ||
        res.m_effective_fragment_size > k_max_payload_size)
        ? static_cast<std::uint16_t>(k_max_payload_size)
...
{
    if (packet.header.m_command != static_cast<std::uint8_t>(CommandId::k_req_login))
    {
        return std::nullopt;
    }
...
{
    if (packet.header.m_command != static_cast<std::uint8_t>(CommandId::k_res_login))
    {
        return std::nullopt;
    }
    const auto& buf = packet.payload;
    ...
    const boost::asio::ip::udp::endpoint& endpoint)
{
    ...
    const std::uint16_t k_effective = (k_requested == 0)
        ? static_cast<std::uint16_t>(k_max_payload_size)
