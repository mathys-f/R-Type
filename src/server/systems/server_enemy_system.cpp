...
void sys::server_enemy_system(EngineContext& ctx, ecs::SparseArray<cpnt::Transform> const& positions,
        ...
        if (pos_opt && vel_opt && enemy_opt && health_opt) {
            ...
            if (pos && vel_opt) {
                ...
                if (pos->x < k_offscreen_left || (health && health->hp <= 0)) {
                    ...
                    pos->x = static_cast<float>(GetRandomValue(
                        static_cast<int>(ctx.window_size.x), static_cast<int>(ctx.window_size.x * k_spawn_multiplier)));
