...
std::byte* SerializedComponent::serialize() const {
    ...
    std::memcpy(buffer + sizeof(ComponentType), &data_size,
                sizeof(uint16_t)); // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)

    // Data
    std::memcpy(buffer + sizeof(ComponentType) + sizeof(uint16_t), data.data(),
...
SerializedComponent SerializedComponent::deserialize(std::byte const* data_ptr) {
    ...
    std::memcpy(&data_size, data_ptr + sizeof(ComponentType),
                sizeof(uint16_t)); // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)

    // Data
    component.data.resize(data_size);
    std::memcpy(component.data.data(),
                data_ptr + sizeof(ComponentType) +
                    sizeof(uint16_t), // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                data_size);
