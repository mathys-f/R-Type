    ...
    for (auto [stats_idx, stats_opt] : ecs::indexed_zipper(stats)) {
        if (stats_opt) {
            ...
            if (stat->level >= k_level_to_appear && stat->boss_active == false) {
                ...
                ctx.registry.add_component(boss, cpnt::Transform{ctx.window_size.x - 400.f, 0.f, 0, 0, 0, 0, 1, 1,
                ...
                ctx.registry.add_component(
                    boss, cpnt::Sprite{{27.0f, 861.0f, 154.0f, 203.0f},
                                       k_boss_scale,
                                       0,
                                       "boss"}); // NOLINT(cppcoreguidelines-avoid-magic-numbers,-warnings-as-errors)
                ctx.registry.add_component(
                    boss, cpnt::Health{100, 100}); // NOLINT(cppcoreguidelines-avoid-magic-numbers,-warnings-as-errors)
    ...
    for (auto [boss_idx, boss_opt, pos_opt] : ecs::indexed_zipper(boss, positions)) {
        if (boss_opt && pos_opt && !boss_opt->roar_active) {
            ...
            if (boss->cooldown_1 <= 0.0f) {
                ...
                float spawn_x =
                    pos->x +
                    400.0f; // Adjust offset // NOLINT(cppcoreguidelines-avoid-magic-numbers,-warnings-as-errors)
