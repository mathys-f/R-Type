...
template <typename T> std::shared_ptr<cpnt::ISyncComponent> extract_sync_component(const std::any& component_any) {
...
static std::unordered_map<std::type_index, Extractor> build_sync_extractors() {
    return {{typeid(cpnt::Bullet), extract_sync_component<cpnt::Bullet>},
