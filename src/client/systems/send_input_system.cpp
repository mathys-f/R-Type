...
void send_input_system(engn::EngineContext& ctx) {
    ...
    if (check_key_in_queue(c.move_up.primary) || check_key_in_queue(c.move_up.secondary))
        up = true;
    if (check_key_in_queue(c.move_down.primary) || check_key_in_queue(c.move_down.secondary))
        down = true;
    if (check_key_in_queue(c.move_left.primary) || check_key_in_queue(c.move_left.secondary))
        left = true;
    if (check_key_in_queue(c.move_right.primary) || check_key_in_queue(c.move_right.secondary))
        right = true;
    if (check_key_in_queue(c.shoot.primary) || check_key_in_queue(c.shoot.secondary))
        shoot = true;
