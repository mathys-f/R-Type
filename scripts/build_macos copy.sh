...
if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
    ...
    echo -e "${YELLOW}Output will be saved to client1.log, client2.log, and server.log${RESET}"
    launch_in_terminal "echo 'Running $build_type ./r-type_client'; ./r-type_client 2>&1 | tee client1.log" "R-Type Client1"
    launch_in_terminal "echo 'Running $build_type ./r-type_client'; ./r-type_client 2>&1 | tee client2.log" "R-Type Client2"
    sleep 1  # Small delay to ensure windows open in order
    launch_in_terminal "echo 'Running $build_type ./r-type_server -p 8080'; ./r-type_server -p 8080 2>&1 | tee server.log" "R-Type Server"
else
